...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # reduce the lr, load the best model
        if epoch in args.lr_steps:
            print('load the best model before reduce lr')
            filename = '%s/%s/ckptbest.pth' % (args.root_model, args.store_name)
...
def adjust_learning_rate(optimizer, epoch, lr_type, lr_steps, warmup=10, follow_pretrain=False):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    if epoch < warmup:
        end_lr = 0.1 ** (len(lr_steps)) * args.lr
        star_lr = args.lr

        lr = (star_lr - end_lr) / warmup * epoch + end_lr
